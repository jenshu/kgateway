...
Rather than using a HTTPRoute as a backendRef, a parent can also delegate to child routes using a label selector, by using the custom group/kind `delegation.kgateway.dev/label` as a backendRef:
```yaml
